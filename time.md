...
### 預期圖表
- **圖 3-1**：20 組輸入下加速版與軟體版 Softmax 之執行時間比較
- **圖 3-2**：每組 Softmax 輸出機率分佈比較（範例選 1~3 組視覺化）
- **圖 3-3**：加速版 Softmax 之時間分解圖（max-subtraction / CORDIC exp / Σ / 牛頓法倒數 / 乘法）
- **表 3-1**：20 組整體執行時間、誤差與加速比統計總表
...
### 預期圖表
- **表 4-1**：本研究 vs 各文獻方案之 PPA 完整比較表（含 Top-1/Top-5 準確率）
- **圖 4-1**：200 組影像之推論延遲分佈
- **圖 4-2**：功耗-延遲 Pareto 圖（與各對照組比較）
- **圖 4-3**：每瓦特效能（TOPS/W）比較長條圖
- **圖 4-4**：分類結果範例展示（隨機抽樣展示成功與失敗案例）
